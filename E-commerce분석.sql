...
서비스 지솏적으로 사용하는지, 핵심가치를 꾸준히 경험하는지 측정

지속적 사용의 측정: 일반적으로 방문을 기준으로 측정
                            특정 페이지 방문
                            결제버튼 클릭 또는 결제 완료
                            특정 기능 사용
클래식 리텐션 : 특정 일에 특정유저군의 몇%가 접속했는지 계산
                      첫 방문 후 특정 일자에 재방문한 유저 비율
                     유저가 최초로 특정한 액션을 취한 날을 Day 0 
                     사용자가 매일 접속해서 사용할 것으로 기대되는 서비스에 활용
                     유저들이 매일 재방문하는 것이 아니라 매주,매달 정기적으로 재방문하면 주,달 리텐션이 적절한 지표가 될것
--클래식 리텐션
with first_order_date  as
(
